...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    ...
    <flux:sidebar stashable sticky
        ...
        <flux:navlist.item icon="layout-grid" :href="route('dashboard.index')"
            :current="request()->routeIs('dashboard')" wire:navigate>
            {{ __('Dashboard') }}
        </flux:navlist.item>
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Categories')">
                ...
                @endphp
                @foreach ($categories as $category)
                    <flux:navlist.item icon="folder" :href="route('categories.show', $category->id)"
                        :label="$category->name" wire:navigate>
                        {{ $category->name }}
                    </flux:navlist.item>
                @endforeach
            </flux:navlist.group>
